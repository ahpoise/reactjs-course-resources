...
import { usersLoading, usersLoaded, usersFailed } from "../store/usersSlice";
import type { UsersState } from "../store/usersSlice";

export function UsersList() {
  const dispatch = useDispatch();
  const users = useSelector((state: { users: UsersState }) => state.users.list);
  const status = useSelector((state: { users: UsersState }) => state.users.status);
  const [selectedTag, setSelectedTag] = useState("");
  const [sortBy, setSortBy] = useState<"name" | "age">("name");

  // Fetch só quando o status é idle: não re-busca (nem pisca o loading) ao voltar à lista.
  useEffect(() => {
